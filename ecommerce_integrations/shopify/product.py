...
from ecommerce_integrations.shopify.constants import SETTING_DOCTYPE, MODULE_NAME, SHOPIFY_VARIANTS_ATTR_LIST
...
class ShopifyProduct:
	...
	def _create_item(self, product_dict, warehouse, has_variant=0, attributes=None, variant_of=None):
		...
		ecommerce_item.create_ecommerce_item(MODULE_NAME,
	...
	def _create_item_variants(self, product_dict, warehouse, attributes):
		template_item = ecommerce_item.get_erpnext_item(MODULE_NAME, integration_item_code=product_dict.get("id"))
			...
			for variant in product_dict.get("variants"):
				...
				for i, variant_attr in enumerate(SHOPIFY_VARIANTS_ATTR_LIST):
					if variant.get(variant_attr):
						attributes[i].update({"attribute_value": self._get_attribute_value(variant.get(variant_attr), attributes[i])})
	...
	def _get_supplier(self, product_dict):
		if product_dict.get("vendor"):
			supplier = frappe.db.sql("""select name from tabSupplier
				where name = %s or shopify_supplier_id = %s """, (product_dict.get("vendor"),
				product_dict.get("vendor").lower()), as_list=1)
