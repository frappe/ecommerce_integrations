...
def temp_shopify_session(shopify_account=None):
	...
	def decorator(func):
		...
		def wrapper(*args, **kwargs):
			...
			# Accept account from kwargs for callers that pass it at runtime.
			runtime_account = kwargs.pop("shopify_account", None)

			# If a callable is passed, call it with self to get the account
			if runtime_account:
				account = runtime_account
			elif shopify_account is None:
			...
			# Recover from accidental character-split account values (e.g. passing *"My Account").
			if (
				isinstance(account, str)
				and len(account) == 1
				and len(args) > 1
				and all(isinstance(arg, str) and len(arg) == 1 for arg in args)
			):
				account = "".join(args)
