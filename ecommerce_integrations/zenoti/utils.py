...
def add_items(doc, item_data):
	for item in item_data:
		...
		if invoice_item.get('item_tax_template'):
			invoice_item['item_tax_template'] = frappe.db.get_value("Item Tax Template", filters={"title": invoice_item['item_tax_template']})
...
def get_item_tax_rate(item_tax_template):
	tax_values = frappe.db.get_value("Item Tax Template Detail", {"parent": item_tax_template}, ["tax_type", "tax_rate"])
	tax_dict = {
		tax_values[0]: tax_values[1]
	}
