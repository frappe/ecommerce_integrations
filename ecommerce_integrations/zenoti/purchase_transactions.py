...
from ecommerce_integrations.zenoti.utils import check_for_item, add_items, add_taxes, api_url, make_api_call, make_address, check_for_item_tax_template, get_cost_center
...
def create_supplier(supplier_name):
	...
	supplier.supplier_group = frappe.db.get_single_value("Zenoti Settings", "default_supplier_group") if frappe.db.get_single_value("Zenoti Settings", "default_supplier_group") else "All Supplier Groups"
...
def get_required_data_to_create_purchase_record(order, error_logs):
	...
	if not frappe.db.exists("Purchase Invoice", {"zenoti_order_no": order['order_number']}) and not frappe.db.exists("Purchase Order", {"zenoti_order_no": order['order_number']}):
		...
		if len(item_err_msg_list):
			item_err_msg = "\n".join(err for err in item_err_msg_list)
			msg = _("For Order no {}.\n").format(order['order_number']) + item_err_msg
			error_logs.append(msg)
