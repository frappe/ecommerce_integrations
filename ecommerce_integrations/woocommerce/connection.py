...
from ecommerce_integrations.woocommerce.constants import SETTING_DOCTYPE

def verify_request():
	woocommerce_setting = frappe.get_doc(SETTING_DOCTYPE)
	sig = base64.b64encode(
		hmac.new(
			woocommerce_setting.secret.encode('utf8'),
			frappe.request.data,
			hashlib.sha256
		).digest()
	)
	...
	frappe.set_user(woocommerce_setting.creation_user)
...
def _order(*args, **kwargs):
	...
	if event == "created":
		sys_lang = frappe.get_single("System Setting").language or 'en'
		...
		link_items(order.get("line_items"), woocommerce_setting, sys_lang)
		create_sales_order(order, woocommerce_setting, customer_name, sys_lang)
...
def link_items(items_list, woocommerce_setting, sys_lang):
	for item_data in items_list:
		...
		item.stock_uom = woocommerce_setting.uom or _("Nos", sys_lang)
		item.flags.ignore_mandatory = True
		item.save()

def create_sales_order(order, woocommerce_setting, customer_name, sys_lang):
	new_sales_order = frappe.new_doc("Sales Order")
	new_sales_order.customer = customer_name

	new_sales_order.po_no = new_sales_order.woocommerce_id = order.get("id")
	new_sales_order.naming_series = woocommerce_setting.sales_order_series or "SO-WOO-"

	created_date = order.get("date_created").split("T")
	new_sales_order.transaction_date = created_date[0]
	delivery_after = woocommerce_setting.delivery_after_days or 7
	new_sales_order.delivery_date = frappe.utils.add_days(created_date[0], delivery_after)

	new_sales_order.company = woocommerce_setting.company

	set_items_in_sales_order(new_sales_order, woocommerce_setting, order, sys_lang)
...
def set_items_in_sales_order(new_sales_order, woocommerce_setting, order, sys_lang):
	company_abbr = frappe.db.get_value('Company', woocommerce_setting.company, 'abbr')

	default_warehouse = _("Stores - {0}", sys_lang).format(company_abbr)
	if not frappe.db.exists("Warehouse", default_warehouse) \
		and not woocommerce_setting.warehouse:
	...
	for item in order.get("line_items"):
		...
		new_sales_order.append("items",{
			...
			"uom": woocommerce_setting.uom or _("Nos", sys_lang),
			"qty": item.get("quantity"),
			"rate": item.get("price"),
			"warehouse": woocommerce_setting.warehouse or default_warehouse
			})

		add_tax_details(new_sales_order, ordered_items_tax, "Ordered Item tax", woocommerce_setting.tax_account)

	# shipping_details = order.get("shipping_lines") # used for detailed order

	add_tax_details(new_sales_order, order.get("shipping_tax"), "Shipping Tax", woocommerce_setting.f_n_f_account)
	add_tax_details(new_sales_order, order.get("shipping_total"), "Shipping Total", woocommerce_setting.f_n_f_account)
