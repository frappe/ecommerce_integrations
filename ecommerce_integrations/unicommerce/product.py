...
UNI_TO_ERPNEXT_ITEM_MAPPING = {
    ...
    "brand": "brand",  # Link Field, might not exist
...
def upload_new_items(force=False) -> None:
    ...
    log = create_unicommerce_log(
        status="Queued",
        message=f"Item sync initiated for {len(new_items)} item(s)",
        make_new=True
    )

    synced_items = upload_items_to_unicommerce(new_items, log=log)
    unsynced_items = set(new_items) - set(synced_items)

    log.message = (
        "Item sync completed\n\n"
        f"✓ Synced ({len(synced_items)}): {', '.join(synced_items) if synced_items else 'None'}\n\n"
        f"✗ Failed ({len(unsynced_items)}): {', '.join(unsynced_items) if unsynced_items else 'None'}"
    )
    log.status = "Success" if synced_items else "Failure"
    log.save()


def _get_new_items() -> List[ItemCode]:
    """Get items that need to be synced to Unicommerce.
    
    🔧 CRITICAL FIX: Changed `item.item_code` to `item.name`
    
    This ensures we fetch the primary key (name field) instead of the item_code field.
    Without this fix, items where name ≠ item_code will crash when passed to 
    frappe.get_doc("Item", item_code) because get_doc() requires the primary key.
    
    Example of the bug:
      - Item has name="BLPBIB0003251" and item_code="BLP-BIB-00001"
      - Old query: SELECT item.item_code returns "BLP-BIB-00001"
      - frappe.get_doc("Item", "BLP-BIB-00001") crashes with "Item not found"
      - Because "BLP-BIB-00001" is NOT the primary key
    
    Fixed query: SELECT item.name returns "BLPBIB0003251"
      - frappe.get_doc("Item", "BLPBIB0003251") works correctly
    
    This bug affected 1,653 items (30% of inventory) in production.
    
    Also fixed JOIN condition to use item.name instead of item.item_code
...
def upload_items_to_unicommerce(
    item_codes: List[ItemCode], client: UnicommerceAPIClient = None, log=None
) -> List[ItemCode]:
    """Upload multiple items to Unicommerce with error handling.
    
    Returns successfully synced item codes.
    ...
    for idx, item_code in enumerate(item_codes, 1):
        try:
            ...
            # Continue with next item instead of crashing entire batch
            continue
...
def _build_unicommerce_item(item_code: ItemCode) -> JsonDict:
    ...
    Note: item_code parameter actually contains the Item's `name` field (primary key)
    due to the fix in _get_new_items(). This allows frappe.get_doc() to work correctly.
    """
...
def _handle_ecommerce_item(item_code: ItemCode) -> None:
    """Create or update Ecommerce Item record after successful sync."""
...
def validate_item(doc, method=None):
    ...
    if not UNICOMMERCE_SKU_PATTERN.fullmatch(item.item_code):
        msg = _("Item code is not valid as per Unicommerce requirements.") + "  \n"
